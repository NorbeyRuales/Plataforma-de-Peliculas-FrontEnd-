...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const api = useMemo<Ctx>(() => ({
    show,
    success: (text, opts) => show({ type: 'success', text, autoHideMs: 2200, ...opts }),
    error:   (text, opts) => show({ type: 'error', text, ...opts }), // cerrar manual
    info:    (text, opts) => show({ type: 'info', text, autoHideMs: 3500, ...opts }),
    warning: (text, opts) => show({ type: 'warning', text, autoHideMs: 4000, ...opts }),
  }), [show])
